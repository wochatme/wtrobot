...
static void print_usage(void)
{
	...
	mosquitto_lib_version(&major, &minor, &revision);
	printf("wtrobot is a simple mqtt client that will subscribe to a set of topics and handle all messages it receives.\n");
	printf("wtrobot version %s running on libmosquitto %d.%d.%d.\n\n", VERSION, major, minor, revision);
	printf("Usage: wtrobot       {[-h host] [--unix path] [-p port] [-u username] [-P password] -t topic | -L URL [-t topic]}\n");
